# KOSPI Shadow Decision Coach v5.5.0

## v5.5 automation integrity

- 정적 앱 셸, 운영 신뢰도 가드, 휴장·장전 상태 가드를 각각 독립 파일로 정식 로드합니다. 서비스워커가 자바스크립트 소스를 조합·변형하지 않습니다.
- 장중 KIS 스냅샷은 10분, 애프터마켓 스냅샷은 20분 목표로 자동 배포합니다. 09:05·15:45·20:05 판단 체크포인트는 별도로 보강합니다.
- 전체 모델 갱신은 07:30·20:05로 분리해 NXT 원시 스냅샷 수집과 충돌하지 않게 했습니다. Pages 배포는 하나의 대기열로 직렬화합니다.
- 앱은 다음 갱신 목표 시각과 실제 KIS 수신 경과시간을 구분해 표시합니다. 수신 시각이 15분을 넘기거나 불완전하면 신규 매매 판단을 잠급니다.

기존 누수 통제형 KOSPI 예측 파이프라인에 **시간대별 시장 코칭 PWA**를 결합한 개인 연구용 앱입니다.

## v5.3 Theme & Supply Radar

- `종목 후보` 화면에 체크포인트 이전 기사·공시, NXT 상대거래대금, 같은 테마 종목의 방향 확산, 전일 NASDAQ·SOX, 전일 종가 기준, 날씨 재료를 분리해 표시하는 관찰 레이더를 추가합니다.
- 테마 구성원은 설정 종목 또는 시각이 확인된 기사·공시에 실제 연결된 종목만 사용합니다. 시장 전체 테마 폭, 포털 조회수·검색순위, 직접 날씨 실측·예보가 연결되지 않으면 `unavailable`로 남깁니다.
- 단일 종목 급등이나 설정된 안전 이격 이상은 `추격 검토`로 표시합니다. 테마 레이더는 확률·진입 신호를 만들지 않으며 KOSPI Gate와 종목 모델을 우회할 수 없습니다.
- 07:30, 08:00, 08:50, 09:05 관찰 스냅샷을 Shadow 원장에 저장하고 향후 5·15·30분 결과를 별도로 검증할 계약을 마련합니다. 검증 전에는 모든 가중치와 성과 지표가 비활성입니다.

### v5.3.1 데이터 커버리지

- KIS KOSPI 거래대금·거래량 증가 순위를 받아 실시간 시장 관심 종목을 표시합니다.
- 명시적인 테마 종목표는 실제 수신된 KIS 순위 종목과 교차할 때만 표시하며 시장 전체 테마 폭으로 과장하지 않습니다.
- 서울 날씨는 Open-Meteo 모델 예보로 수신하고 공식 특보나 매매 신호가 아님을 표시합니다.
- 기사 제목 끝의 매체명을 제거한 뒤 테마를 분류해 `조선일보`가 조선 테마로 잡히는 오탐을 막습니다.
