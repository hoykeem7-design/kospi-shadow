...
| News | Broad feed with configured-symbol title matching | Google News RSS | Existing broad query | Article | Current deployment only | KST normalization/dedup implemented | Yes; relation only when name/code appears | None | unknown/date-only retained | Time and source shown |
| Official disclosure | Yes with optional key | OpenDART | `list.json`, configured symbols only | Date/event | Current deployment and decision history | Optional interface implemented | Only with `DART_API_KEY` | None | `DART_API_KEY_NOT_CONFIGURED`/unavailable | Exact time not invented |
...
| NXT aftermarket price/volume/turnover | Yes for configured symbols when KIS responds | KIS | `inquire-price`/book, market `NX`, after 15:40 | Snapshot | Accumulated after v5 | Implemented | Yes when received | None | `null/unavailable` | Data not received |
| NXT aftermarket same-time relative metrics | Derived after 20 valid dates | Durable history | `history/raw/<symbol>.jsonl` | Backward-only same-minute snapshot | Starts after activation | Median implementation | After baseline accumulation | None | `relative_value=null` | Baseline data insufficient |
